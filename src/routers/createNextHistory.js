...
function fromReactRouterToNextUrl(to) {
...
export function createNextHistory(asPath) {
	const historyMemory = createMemoryHistory({ initialEntries: [asPath] });
	...
	const enhancedHistory = {
		...
		listen(listener) {
			function handleRouteChange(_url) {
				listener({
	...
	function handleRouteChangeFromNext(url) {
